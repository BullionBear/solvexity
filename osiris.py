...
async def setup_jetstream_consumer(js, config: OsirisConfig):
    ...
    try:
        ...
        # Convert opt_start_time to RFC3339 string
        # IMPORTANT: Despite the type hint saying Optional[int], the NATS server expects RFC3339 string!
        # Supports: RFC3339 string ("2025-10-10T09:00:00Z") or Unix timestamp in milliseconds (1760086800000)
        ...
        if not opt_start_time or opt_start_time == 0:
            opt_start_time = None
        elif isinstance(opt_start_time, str):
            # Already RFC3339 string, validate and keep it
            dt = datetime.fromisoformat(opt_start_time.replace('Z', '+00:00'))
            logger.info(f"Using opt_start_time '{opt_start_time}' ({dt} UTC)")
            # Keep as string - NATS server expects RFC3339 format
        elif isinstance(opt_start_time, (int, float)):
            # Unix timestamp in milliseconds, convert to RFC3339 string
            dt_utc = datetime.fromtimestamp(opt_start_time / 1000, tz=timezone.utc)
            opt_start_time = dt_utc.isoformat().replace('+00:00', 'Z')
            logger.info(f"Converted opt_start_time {config.consumer.opt_start_time} ms to RFC3339 '{opt_start_time}' ({dt_utc} UTC)")
...
async def main(config_path: str = "config/osiris.json"):
    ...
    async def on_trade(e: Event):
        logger.info(f"Received trade: {e.data}")
        dt = datetime.fromtimestamp(e.data.timestamp / 1000)
        logger.info(f"Trade datetime: {dt}")
        """
