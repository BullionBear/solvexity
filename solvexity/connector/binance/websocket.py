...
class BinanceWebSocketSubscriber:
    ...
    async def _handle_websocket(self, channel: str, callback: Callable[[Dict], None], max_retries: int = 5):
        ...
        while retry_count <= max_retries:
            try:
                logger.info(f"Connecting to WebSocket: {channel} (attempt {retry_count + 1}/{max_retries + 1})")
                ...
                if retry_count < max_retries:
                    ...
                    logger.error(f"Max retries ({max_retries}) reached for {ws_raw_url}. Stopping reconnection attempts.")
                    break
        ...
        if retry_count > max_retries:
            logger.error(f"Failed to establish WebSocket connection after {max_retries} retries for {ws_url}")

    async def subscribe(self, ws_url: str, callback: Callable[[Dict], None], max_retries: int = 5) -> Callable[[], None]:
        """Subscribe to a websocket stream non-blocking"""
        self.callbacks[ws_url] = callback
        
        task = asyncio.create_task(self._handle_websocket(ws_url, callback, max_retries))
